...
ISM43362::ISM43362(PinName tx, PinName rx, bool debug)
    : _serial(tx, rx, ISM43362_DEFAULT_BAUD_RATE),
      _parser(&_serial),
      _packets(0),
      _packets_end(&_packets)
{
    _serial.set_baud( ISM43362_DEFAULT_BAUD_RATE );
...
bool ISM43362::dhcp(bool enabled, int mode)
{
    //only 3 valid modes
    if(mode < 0 || mode > 2) {
        return false;
